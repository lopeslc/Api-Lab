...
#--------------------------------------------FIM CLASSE CADASTRO ESTUDANTES--------------------------------------------
...
class CadastroAdministrador:
    ...
    def __init__(self, root):
        self.root = root
        self.root.geometry("500x500")
        self.root.title("Cadastrar Administrador")
        self.root.resizable(False, False)
        self.root.config(bg="#fff", bd=10)
        
        # Titulo da pagina
        title_page = Label(root, text="AppLab ADMINISTRADORES", fg="ORANGE", font=("Comic Sans", 13, "bold"), pady=5)
        title_page.pack()
        ...
        # Marco
        marco = LabelFrame(root, text="dados do ADMINISTRADOR", font=("Roboto", 10, "bold"))
        marco.config(bd=2, pady=5)
        marco.pack()
    ...
    def registrar_estudante(self):
        if self.validar_formulario_completo() and self.validar_password() and self.validar_email_institucional():
            ...
            messagebox.showinfo("ADMINISTRADOR CADASTRADO", f"""
                                Username: {self.username.get()}
                                Email institucional: {self.email_institucional.get()}
                                Senha: {self.password.get()}
                                Recuperação: {self.recovery_question.get()}
...
#--------------------------------------------FIM CLASSE CADASTRO ADMINISTRADORES--------------------------------------------
...
def tela_cadastro_estudante():
    root = Tk()
    app = CadastroEstudante(root)
    root.mainloop()

def tela_cadastro_admin():
    root = Tk()
    app = CadastroAdministrador(root)
    root.mainloop()
